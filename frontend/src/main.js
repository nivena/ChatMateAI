...
import {SendMsg, SendNumber, InitWhatsApp} from '../wailsjs/go/main/App';
...
// Send the message and the language to be used
const msg_send = async () => {
    ...
    if (msg === "") return;
    try{
        // Call App.SendMsg(name)
        const result = await SendMsg(msg );

        // Update result with data back from App.SendMsg()
